...
export default class ETH {
  ...
  async getBalance(address: string, blockNumberOrTag: number | RPCTAG) {
    return this.connection.rpc.send(
      'eth_getBlockByHash',
      address,
      hexOrQuantifier(blockNumberOrTag),
    );
  }
  ...
  async getBlockByHash(blockHash: string, includeTransactions = false) {
    return this.connection.rpc.send(
      'eth_getBlockByHash',
      hexOrQuantifier(blockHash),
      includeTransactions,
    );
  }

  async getBlockByNumber(
    blockNumber: string | number | RPCTAG,
